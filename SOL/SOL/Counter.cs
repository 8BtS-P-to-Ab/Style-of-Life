...
namespace SOL
{
    public partial class Counter : Form
    {
		...
		string path = Path.Combine(Environment.CurrentDirectory, @"..\..\Resources\Counter\");     //get the current working directory
		FileInfo fileI;
        ...
        {
            if (!string.IsNullOrWhiteSpace(txtBxCounterName.Text)) {                                        //Tif the new counter's name is not empty or whitespace 
                char[] invalid = Path.GetInvalidFileNameChars();                                            //|--get all file related invalid characters
                                                                                                            //|
                for (int i = 0; i < invalid.Length; i++) {                                                  //|-Tfor loop until looped through all the avalid characters
                    txtBxCounterName.Text = txtBxCounterName.Text.Replace(invalid[i], '-');                 //|-|----replace all invalid characters with '-'
                                                                                                            //| |
                }                                                                                           //|-|L)
                                                                                                            //|
                string directoryPart = Path.Combine(@"..\..\..\", txtBxCounterName.Text + ".txt");          //|--get the file's full path
                                                                                                            //|
                if (!File.Exists(Path.Combine(Environment.CurrentDirectory, directoryPart))) {              //|-Tif file doesn't exist
                    var f = File.Create(Path.Combine(Environment.CurrentDirectory, directoryPart));         //|-|----create the file in the working directory
                    f.Close();                                                                              //|-|----close the file
                    updateList();                                                                           //|-|----update the list so that the new counter is selectable 
                                                                                                            //| |
                } else {                                                                                    //|-\c)if file already exists
                    if (MessageBox.Show("File \"" + txtBxCounterName.Text + ".txt"                          //|-|----prompt to replace
                        ...
                        MessageBoxIcon.Warning) == DialogResult.Yes) {                                      //|-|---Tif response is yes
                        int selectedIndex = lstBxCounterSelector.FindString(txtBxCounterName.Text);         //|-|---|----get the index of file that already exists
                                                                                                            //| |   |
                        var f = File.Create(Path.Combine(Environment.CurrentDirectory, directoryPart));     //|-|---|----replace the file (clearing it)
                        f.Close();                                                                          //|-|---|----close the file 
                        ...
                        lstBxCounterSelector.SelectedIndex = selectedIndex;                                 //|-|---|----select the counter item
                                                                                                            ...
                                                                                                            //|
            } else {                                                                                        //\c)otherwise if the new counters name is not given
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            int selectedIndex = lstBxCounterSelector.SelectedIndex;             //get the index the selected counter item is
                                                                                //
            if (!string.IsNullOrWhiteSpace(selected)) {                         //Tcheck if a selection has been made, if so;
                if (File.Exists(localPath)) {                                   //|-Tcheck if the file already exists, if it does;
                                                                                //| |
                    ...
                    lstBxCounterSelector.SelectedIndex = selectedIndex;         //|-|----reselect the counter item (because the display updated)
                                                                                //| |
                } else {                                                        //|-\c)otherwise if the file does not exist
                    MessageBox.Show("Couldn't find the file to update.");       //|-|----tell the user
                                                                                //| |
                }                                                               //|-|e)
            } else {                                                            //\c)otherwise if no item is selected
        ...
        {
            AboutCount test = new AboutCount();
		...
		private void updateList()
        {
            ...
            string[] files = System.IO.Directory.GetFiles(path, "*.txt");           //get every counter's .txt file (where the data is stored)
                                                                                    //
            if (files.Length != 0) {                                                //Tif there is at least one file in the working directory folder
                foreach (string file in files) {                                    //|-Tforeach item in the 'files' array get each file as 'file'
		...
		private void removeSelected()
        {
            string selected = lstBxCounterSelector.Text + ".txt";                                   //get the currently selected counter
                                                                                                    //
            try {                                                                                   //Tcatch any errors of this:
                if (File.Exists(path + selected)) {                                                 //|-Tif the file exists
                    File.Delete(path + selected);                                                   //|-|----delete it
                    updateList();                                                                   //|-|----update the list view
                                                                                                    //| |
                } else {                                                                            //|-\c)otherwise if the file doesn't exist
            ...
            } catch (Exception) {                                                                   //|c)catch any exception
		...
		private void updateCounter(string modifyer, string localPath)
        {
            ...
            {                                                   //Tif a counter is selected (null not selected)
                ...
                if (modifyer == "++") {                         //|-Tif the modifier is itterate
                    countLocal++;                               //|-|----itterate the tracker
                } else if (modifyer == "--") {                  //|-\c)otherwise if the modifier is decrament
        ...
        private void btnRemoveCounter_Click_1(object sender, EventArgs e)
        {
            removeSelected();
        }
    }
